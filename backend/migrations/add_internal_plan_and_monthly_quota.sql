...
-- ========================================
-- This migration:
-- 1. Adds 'internal' plan type support
-- 2. Adds monthly quota columns for monthly plans
-- 3. Updates plan constraints
--
-- IMPORTANT: This migration does NOT create RLS policies.
...
-- Step 3: Update user_plans table to allow 'internal' plan
ALTER TABLE user_plans 
DROP CONSTRAINT IF EXISTS user_plans_plan_check;

ALTER TABLE user_plans 
ADD CONSTRAINT user_plans_plan_check 
CHECK (plan IN ('start', 'normal', 'high', 'ultra', 'internal'));
